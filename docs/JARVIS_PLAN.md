...
> **RESUME HERE (updated 2026-07-15, at commit `d183b36`).**
> The entire Apex A–E critical path, the E7 ship gate, and F1 (F1a–F1c), F2
> (F2a–F2b), F3a+F3b, and **F3-hardening (companion private key → Keychain)** are
> complete, committed, and pushed. Every apex gate `a1`–`f3b` plus `f3hardening`
> and `e7` is green; 470 backend + 37 frontend + 4 Node relay tests pass; the
> daemon boots clean against the real production DB. **The first unfinished work
> is F3c**, and it is intentionally not started — read its row below before
> touching it. **Recommended next action for a fresh session:** raise F3c's two
> blocking decisions with the user before scoping — (1) client shape
> (reference-Rust vs a loadable PWA-over-relay vs a native iOS/Android app, which
> is a separate codebase) and (2) the audio path (remoting the C4 `VoiceSession`
> is realtime-audio + hardware-gated; the tractable in-repo slice is an
> Opus/PCM-frame method on the existing companion protocol with a reference
> client loop). Do not start F3c cold; it will hit the same wall. Locked F3
> decisions (do not re-litigate): reference Rust client now, Noise via `snow`.
> Standing rule: after each milestone, run its `scripts/apex_<id>_check.sh`,
> launch the built binary against the real
...
| F3 | Voice companion (phone/earbuds) | F1c, C4 | in progress; F3a (encrypted channel + store-backed surface) + F3b (ciphertext-only relay) + F3-hardening (private key -> Keychain) done; F3c (audio + real client) open and BLOCKED on a product decision. Decisions taken at F3 kickoff: reference Rust client now (a native mobile app is a separate codebase that speaks this proven protocol), Noise via the `snow` crate. The Pillar 14 relay-compromise exit criterion (relay sees ciphertext only) is met at F3b |
