...
> (audio remoting over the companion protocol)**. Every apex gate `a1`–`f3b`
> plus `f3hardening`, `f3c`, and `e7` is green; 475 backend + 37 frontend + 4
> Node relay tests pass; the daemon boots clean against the real production DB.
> The two remaining F items are **deferred-by-decision, not unfinished**:
...
| F3 | Voice companion (phone/earbuds) | F1c, C4 | complete for everything verifiable in-repo; F3a (encrypted channel + store-backed surface) + F3b (ciphertext-only relay) + F3-hardening (private key -> Keychain) + F3c (audio remoting over the protocol) done. The remaining piece, F3c-client (a real loadable phone client + live realtime audio), is deferred-by-decision -- gated on the user committing to a phone target, not on engineering readiness. Decisions taken at F3 kickoff: Noise via the `snow` crate; reference Rust client now, with a native app sharing that Rust core the recommended shape when F3c-client is picked up. The Pillar 14 relay-compromise exit criterion (relay sees ciphertext only) is met at F3b and now holds for audio too (F3c) |
...
| F3-hardening | Companion static key -> Keychain | F3a | complete; `scripts/apex_f3hardening_check.sh` passed. The companion Noise static PRIVATE key moved out of `app_settings` into the Keychain, next to the api keys. `secrets.rs` gained a `CompanionKeyStore` trait + `SystemCompanionKeyStore` Keychain impl (account `companion_static_private_key` under the existing `com.jeff.desktop` service), mirroring the OpenAI/Anthropic pattern. `companion::identity()` is now Keychain-backed via a pure `decide_identity` (UseKeychain / Migrate / Generate) + `identity_with_key_store`: on a pre-hardening store it migrates the DB private key into the Keychain and `delete_app_setting`s the DB copy on first read, keeping the public key; generation writes private->Keychain, public->DB, and never writes the private key to `app_settings`. The public key, pairing psk, and rendezvous token stay in the DB as routing material. `identity(store)` signature unchanged (delegates to the system store), so `begin_pairing`/`serve_one_via_relay` did not ripple; extracted `open_pairing_window` so the authorization test opens a window without minting a keychain-backed identity. Keychain access is gated exactly like the existing key paths -- no unit test touches the live Keychain; the migration decision + fallback are tested deterministically via a `MockKeyStore`. Plus warning-free `cargo check`, 5 f3-hardening tests (decision table; migrate-and-clear + idempotent; generate-into-keychain-never-DB; use-keychain-without-rewriting-DB; read-failure surfaces rather than forging an identity), 460 backend lib tests (5 new, 470 total incl. other suites), f3a/f3b gates still green, both bins build, and a real-DB boot smoke (daemon opens the 764KB production `jeff_store.sqlite3` headless with no panic; identity() is not called on boot -- only via the pairing command or the relay scheduler, both off by default -- so a normal boot performs no Keychain access). Live Keychain read/write is env/hardware-gated like the other key paths |
| F3c | Audio remoting over the companion protocol | F3b, C4 | complete (transport slice; live audio + real client deferred-by-decision, see F3c-client); `scripts/apex_f3c_check.sh` passed. A voice turn now streams typed `AudioFrame`s (codec/sample_rate/base64 payload/end) over the same Noise session as `turn`/`recall`/`jobs`, so audio is encrypted and relayed as opaque ciphertext by construction -- the Pillar 14 relay property holds for audio for free. `dispatch` acks a `voice` open on the request/response channel; `serve` then hands the session to `serve_voice_turn`, which reads the caller's utterance frames (bounded by `accept_audio_frame`: per-frame `MAX_AUDIO_PAYLOAD` 16KiB + per-turn frame/byte budgets so a peer cannot stream unbounded audio into RAM) until an end-marked frame, runs the bridge, streams Jeff's reply frames back, then resumes the request/response loop. `VoiceBridge` is the transport/brain seam: `LoopbackVoiceBridge` (in-repo default, deterministic frame-for-frame echo) proves the pipe; `RealtimeVoiceBridge` remotes C4's realtime session and is reached only under an explicit `JEFF_COMPANION_VOICE_LIVE=1` opt-in (never merely because a key is present), so tests never touch the network. Reference `CompanionClient::voice_turn` drives it end to end and runs unchanged through the F3b relay. Half-duplex per turn (full-duplex barge-in is a live-audio property, deferred with the real client; the frame type supports it later). Privacy Center companion disclosure updated to name voice/audio. Plus warning-free `cargo check`, 5 f3c tests (round-trip + session resumes; empty-utterance closes; audio-payload ciphertext-on-wire; per-frame/per-turn budget rejection; live bridge gated off by default), 475 backend lib tests (15 in the f3 suite), f3a/f3b/f3-hardening gates still green, frontend lint, both bins build, and a real-DB boot smoke (daemon opens the 764KB production DB headless with no panic; the voice path is unreachable on boot -- it lives inside an authorized, enabled-only session). Live realtime audio round-trip stays env/hardware-gated like C4 |
| F3c-client | Real loadable phone client + live audio round-trip | F3c | deferred-by-decision (not blocked on engineering readiness). The audio protocol, relay, and encryption are done and proven; what remains is a genuinely loadable client and a live realtime-audio round-trip, both of which need external commitments this repo cannot manufacture and cannot verify in-repo. RESUME WHEN: the user commits to shipping a phone target. Recommended client shape at that point: a native app with a SHARED RUST CORE (compile the existing `companion.rs` client + `snow` to iOS/Android) rather than a browser PWA -- reusing the exact proven Noise_XXpsk3 implementation instead of reimplementing it in JS/WASM on the security-critical path. The live audio bridge already has its seam (`RealtimeVoiceBridge` + `JEFF_COMPANION_VOICE_LIVE`); wiring it to a real `RealtimeVoiceSession` audio stream is the hardware-gated half |
| F4 | Cloud continuation (opt-in) | F1c | deferred-by-decision (post-Apex). This is the one place the plan consciously relaxes the local-first invariant (jobs that outlive the sleeping machine run in an encrypted cloud enclave), so it is not built speculatively: it needs a real cloud-enclave target and an explicit decision to relax the boundary. Pillar 14's exit criteria are already met without it (daemon survives UI crash + OS restart; relay ciphertext-only). RESUME WHEN: the user commits to a cloud target and to relaxing local-first, with the boundary change documented in the Privacy Center before the toggle ships (controls precede capability). With F1/F2/F3 done and F3c-client + F4 deferred-by-decision, Epoch F's in-repo critical path is complete |
