...
| F2 | Overnight work and morning readiness | F1c | re-scoped into F2a + F2b when the epoch began (F-milestones are directional) |
...
| F2b | Morning briefing prepared overnight (retrieval, not on-demand) | F2a | not started; the daemon composes and persists the day's briefing ahead of first engagement -- incorporating overnight work (completed jobs, standing-job receipts, yesterday's consolidation), workload, pending approvals, a store-persisted calendar snapshot -- so the morning briefing is a retrieval. `maybe_fire_briefing` becomes retrieval-first with the existing compose-on-demand path preserved as fallback. Privacy Center morning-readiness surface |
