...
// ---- background autonomous monitor ------------------------------------------

/// spawn the ambient monitor loop as a background task.
/// checks env var JEFF_DISABLE_AMBIENT_MONITOR to skip in CI.
/// runs immediately after a 30-second startup grace period, then every 60 seconds.
pub fn spawn_ambient_monitor<R: Runtime + 'static>(handle: AppHandle<R>) {
    if std::env::var("JEFF_DISABLE_AMBIENT_MONITOR").is_ok() {
        return;
    }
    tauri::async_runtime::spawn(async move {
        // give other startup tasks (watcher, stale-check, etc.) time to settle.
        tokio::time::sleep(std::time::Duration::from_secs(30)).await;
        loop {
            run_monitor_cycle(&handle).await;
            tokio::time::sleep(std::time::Duration::from_secs(60)).await;
        }
    });
}

async fn run_monitor_cycle<R: Runtime>(handle: &AppHandle<R>) {
    let quiet = handle
        .try_state::<AmbientState>()
        .map(|s: tauri::State<'_, AmbientState>| s.is_quiet_mode())
        .unwrap_or(false);

    let Some(jeff) = handle.try_state::<JeffState>() else {
        return;
    };

    crate::synthesis::run_synthesis_check(handle).await;

    // apex c3: daily briefing / debrief rituals ride the same ambient tick.
    crate::briefing::maybe_fire_rituals(handle).await;
