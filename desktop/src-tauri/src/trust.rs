...
pub fn set_trust_level(
    ...
    let current = load_record(store, &class)?;
    upsert_record(
        store,
        &class,
        requested,
...
pub fn demote_trust_class(store: &TaskStore, class: &str) -> Result<TrustLevelDto> {
    let class = normalize_class(class);
    upsert_record(store, &class, TrustLevel::L1, 0, None, true)?;
    trust_level_dto(store, &class)
}

pub fn record_receipt_outcome(store: &TaskStore, receipt: &ActionReceiptDto) -> Result<()> {
    match receipt.status.as_str() {
        "applied" | "approved" => record_approval(store, &receipt.class),
        "reverted" => {
            demote_trust_class(store, &receipt.class)?;
            Ok(())
        }
        "rejected" => reset_approval_streak(store, &receipt.class),
        _ => Ok(()),
    }
}
...
fn record_approval(store: &TaskStore, class: &str) -> Result<()> {
    let class = normalize_class(class);
    let current = load_record(store, &class)?;
    let cap = hard_cap_for_class(&class);
    ...
    let streak = current
        ...
        .unwrap_or(0)
        + 1;
    let offer = if streak >= TRUST_GRADUATION_STREAK
        && current_level == TrustLevel::L1
        && cap >= TrustLevel::L2
    {
        current
            .as_ref()
...
fn reset_approval_streak(store: &TaskStore, class: &str) -> Result<()> {
    let class = normalize_class(class);
    let current = load_record(store, &class)?;
    upsert_record(
        store,
...
fn trust_level_dto(store: &TaskStore, class: &str) -> Result<TrustLevelDto> {
    ...
    {
        Some(format!(
            "Offer L2 for {} after {} consecutive approvals; L3 remains Privacy Center only.",
...
mod tests {
    ...
    fn d4_ten_approvals_offer_l2_never_l3() {
        ...
        let ladder = trust_level_dto(&store, "doc.replace").unwrap();
        ...
        assert!(set_trust_level(&store, "doc.replace", "L3", false).is_err());
    }
    ...
    fn d4_revert_demotes_to_sticky_l1() {
        ...
        set_trust_level(&store, "file.write", "L2", true).unwrap();
