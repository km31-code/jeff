...
const DEADLINE_TERMS: &[&str] = &[
    "deadline", "due", "by friday", "by monday", "tomorrow", "eod", "end of day",
    "by end of", "expires", "closes",
];
const URGENCY_TERMS: &[&str] = &[
    "urgent", "asap", "action required", "please review", "needs your reply",
    "response needed", "reply needed", "time sensitive", "important",
];
const NOISE_SENDERS: &[&str] = &[
    "no-reply", "noreply", "newsletter", "notifications", "digest", "updates@",
    "marketing", "promo",
];
const NOISE_TERMS: &[&str] = &[
    "unsubscribe", "% off", "sale", "webinar", "new features", "weekly digest",
    "your receipt", "promotion", "limited time",
];
...
pub fn triage_score(message: &EmailMessage, context: &TriageContext) -> (i32, String) {
    ...
    if context
        .known_people
        .iter()
        .any(|person| !person.trim().is_empty() && sender_lower.contains(&person.to_ascii_lowercase()))
    {
        score += 2;
    ...
    if NOISE_SENDERS.iter().any(|marker| sender_lower.contains(marker)) {
        score -= 3;
    }
...
pub fn register_reply_watch(
    ...
    if sender.is_empty() {
        return Err(anyhow::anyhow!("reply watch requires a sender"));
    }
...
pub fn message_matches_watch(message: &EmailMessage, watch: &EmailReplyWatchDto) -> bool {
    ...
    let matches_sender = sender.contains(&watch.sender.to_ascii_lowercase());
    let matches_thread = watch.thread_hint.is_empty()
        || message
            .thread_id
            .eq_ignore_ascii_case(&watch.thread_hint)
        || message
            .subject
            .to_ascii_lowercase()
...
pub fn draft_reply(
    ...
    let class = crate::action_bus::ActionClass::EmailDraft.as_str();
...
mod tests {
    ...
    fn e3_triage_flags_matters_and_skips_noise() {
        let ctx = context();
        let matters = msg("1", "Sarah Lee", "Draft review deadline", "The review is due Friday.");
        let noise = msg("2", "no-reply@newsletter.com", "Weekly digest", "Unsubscribe anytime.");
    ...
    fn e3_reply_watch_registers_and_landed_reply_resolves() {
        ...
        register_reply_watch(&store, task_id, "Sarah", "").unwrap();
        let unrelated = msg("1", "Bob", "hi", "hello");
        assert!(resolve_landed_reply(&store, &unrelated).unwrap().is_empty());
        let landed = msg("2", "Sarah Lee", "Re: draft", "here it is");
    ...
    fn e3_draft_reply_is_email_draft_receipt_never_send() {
        let (_dir, store, task_id) = test_store();
        let receipt = draft_reply(&store, task_id, "", "sarah@x.com", "Re: draft", "Slips to Thursday.")
            .unwrap();
