...
pub fn within_speculation_budget(store: &TaskStore) -> bool {
    let spent = store
        ...
        .unwrap_or(0.0);
    let budget = cost_governor::get_daily_budget_usd(store, SPECULATION_BUDGET_KEY)
        .unwrap_or_else(|_| cost_governor::default_daily_budget_usd(SPECULATION_BUDGET_KEY));
    spent < budget
}

pub fn under_daily_prediction_cap(store: &TaskStore) -> bool {
    predicted_today(store).unwrap_or(0) < SPECULATION_DAILY_PREDICTION_CAP
}
...
pub fn should_run_speculation(store: &TaskStore, idle_seconds: i64, now: i64) -> bool {
    ...
    within_speculation_budget(store) && under_daily_prediction_cap(store)
}
...
pub fn run_speculation_cycle(
    ...
    let detail = agent_runtime::create_and_run_job(store, task_id, &top.request, None, true)?;
    ...
    append_event(store, EVENT_PREDICTED, Some(&top.signature))?;
    Ok(Some(cache_id))
}
...
pub fn serve_speculation(
    ...
    let row = conn
        .query_row(
            "SELECT id, request_text, artifact_json
             ...
             WHERE request_signature = ?1 AND status != ?2
             ORDER BY id DESC LIMIT 1",
            params![signature, STATUS_INVALIDATED],
            |row| {
                Ok((
                    row.get::<_, i64>(0)?,
                    row.get::<_, String>(1)?,
    ...
    match row {
        Some((cache_id, request_text, artifact_json)) => {
            ...
            append_event(store, EVENT_HIT, Some(&signature))?;
        ...
        None => {
            append_event(store, EVENT_MISS, Some(&signature))?;
            Ok(None)
        }
    }
}
...
pub fn invalidate_for_task(store: &TaskStore, task_id: i64) -> Result<usize> {
    ...
    let changed = conn
        .execute(
            ...
            params![STATUS_INVALIDATED, task_id, STATUS_FRESH],
        )
        .context("failed to invalidate speculation cache for task")?;
    drop(conn);
    if changed > 0 {
        append_event(store, EVENT_INVALIDATED, None)?;
    }
    Ok(changed)
}
...
pub fn invalidate_stale(store: &TaskStore, now: i64) -> Result<usize> {
    ...
    let changed = conn
        .execute(
            ...
            params![STATUS_INVALIDATED, STATUS_FRESH, cutoff_rfc],
        )
...
pub fn speculation_status(store: &TaskStore) -> Result<SpeculationStatusDto> {
    let (predicted_count, hit_count, miss_count) = event_counts_today(store)?;
    let fresh_cached = fresh_cached_count(store)?;
...
pub fn discard_speculation_entry(store: &TaskStore, cache_id: i64) -> Result<()> {
    let conn = store.connect()?;
    conn.execute(
        "DELETE FROM speculation_cache WHERE id = ?1",
        params![cache_id],
    )
...
fn append_event(store: &TaskStore, kind: &str, signature: Option<&str>) -> Result<()> {
    let conn = store.connect()?;
    conn.execute(
        "INSERT INTO speculation_events (kind, request_signature) VALUES (?1, ?2)",
...
fn predicted_today(store: &TaskStore) -> Result<i64> {
    let conn = store.connect()?;
    conn.query_row(
        "SELECT COUNT(*) FROM speculation_events
         WHERE kind = ?1 AND date(created_at) = date('now')",
        params![EVENT_PREDICTED],
        |row| row.get(0),
    )
    .context("failed to count predictions today")
}

fn event_counts_today(store: &TaskStore) -> Result<(i64, i64, i64)> {
    let conn = store.connect()?;
...
fn fresh_cached_count(store: &TaskStore) -> Result<i64> {
    let conn = store.connect()?;
    conn.query_row(
        "SELECT COUNT(*) FROM speculation_cache WHERE status = ?1",
        params![STATUS_FRESH],
        |row| row.get(0),
    )
...
pub fn maybe_run_for_active_task(
    ...
    let predictions = predict_via_model(router, &recent, goal.as_deref())
        .unwrap_or_else(|_| deterministic_predictions(&recent, goal.as_deref()));
...
fn predict_via_model(
    ...
    let prompt = build_prediction_prompt(recent_requests, goal);
...
fn build_prediction_prompt(recent_requests: &[String], goal: Option<&str>) -> String {
...
mod tests {
    ...
    fn d8_document_delta_invalidates_cache_entry() {
        let (_dir, store, task_id) = test_store();
        let predictions = vec![SpeculationPrediction::new("Check my citations", 0.8)];
        ...
        assert!(serve_speculation(&store, "check my citations").unwrap().is_none());
        let status = speculation_status(&store).unwrap();
