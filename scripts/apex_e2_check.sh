...
for t in \
  ...
  e2_user_name_guard_blocks_matching_query; do
  grep -q "fn $t" "$WEB" || fail "expected e2 test $t is missing"
done
E2_TEST_OUT=$(cd "$ROOT_DIR/desktop/src-tauri" && cargo test e2_ --quiet 2>&1)
...
bash "$ROOT_DIR/scripts/apex_e1_check.sh" >/dev/null 2>&1 || fail "apex e1 tool bus gate regressed"
