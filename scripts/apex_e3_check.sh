...
grep -q "maybe_fire_awaited_reply_landed" "$COMMANDS" || fail "awaited-reply crisis not wired to the landed-reply path"
...
for t in \
  ...
  e3_draft_reply_is_email_draft_receipt_never_send; do
  grep -q "fn $t" "$GMAIL" || fail "expected e3 test $t is missing"
done
E3_TEST_OUT=$(cd "$ROOT_DIR/desktop/src-tauri" && cargo test e3_ --quiet 2>&1)
...
for cmd in triage_inbox summarize_email_thread register_email_reply_watch list_email_reply_watches draft_email_reply notify_email_landed; do
...
bash "$ROOT_DIR/scripts/apex_e2_check.sh" >/dev/null 2>&1 || fail "apex e2 web research gate regressed"
